...
public class ExpandingArrow : MonoBehaviour
{
    Rigidbody2D rb;
    bool hasHit;
    ...
    void DuplicateArrow()
    {
        ...
        //newArrow.GetComponent<Rigidbody2D>().velocity = new Vector3(launchForce, 1, 0);
    }
    void DuplicateArrow2()
    {
        GameObject newArrow = Instantiate(arrow, pos2.position, pos2.rotation);
        newArrow.GetComponent<Rigidbody2D>().velocity = transform.right * launchForce;
        //newArrow.GetComponent<Rigidbody2D>().velocity = new Vector3(launchForce, 0, 0);
    }
